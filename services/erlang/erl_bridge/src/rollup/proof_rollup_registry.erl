%%%-------------------------------------------------------------------
%%% @doc ProofRollup Registry
%%% Keeps track of active rollup instances managed by proof_rollup_sup.
%%% Allows registration, lookup, unregistration, and listing of rollups.
%%%-------------------------------------------------------------------

-module(proof_rollup_registry).
...
init([]) ->
    io:format("[ProofRollupRegistry] started~n"),
    {ok, #{}}.

handle_call({register, RollupId, Pid}, _From, State) ->
    io:format("[ProofRollupRegistry] Registering ~p -> ~p~n", [RollupId, Pid]),
    {reply, ok, maps:put(RollupId, Pid, State)};

handle_call({unregister, RollupId}, _From, State) ->
    io:format("[ProofRollupRegistry] Unregistering ~p~n", [RollupId]),

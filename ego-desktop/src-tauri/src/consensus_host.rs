...
///
/// VERIFIED LINCHPIN (2026-06-26): `ego_core::KeyPair::public_key()` returns the
/// **Dilithium2 (ML-DSA-44)** key and `KeyPair::sign()` is a Dilithium signature — NOT
/// ed25519. So the engine's address space is `blake3(dilithium_pk)[..20]`. The
/// `validator_set` MUST therefore be built from each validator's **Dilithium** public
/// key: self via `local_keypair.public_key()`, peers via the on-chain BFT pubkey
/// registry (`register_validator_pubkey` stores the Dilithium2 key). Do NOT use
/// ed25519 here — see `engine_address_is_dilithium_derived` test. (The desktop's bech32
/// address is a *separate*, ed25519-derived identity; map bech32 <-> engine-Address via
/// the per-validator pubkey lookup, never by re-deriving.)
pub fn consensus_address(pk: &PublicKey) -> Address {
    Address::from_public_key(pk)
}

/// Build an engine `Address` from a validator's Dilithium2 (ML-DSA-44) public key bytes
/// (as stored in the desktop's BFT validator-pubkey registry).
pub fn address_from_dilithium(dilithium_pk: Vec<u8>) -> Address {
    Address::from_public_key(&PublicKey::dilithium2(dilithium_pk))
}
...
mod tests {
    ...
    /// LINCHPIN: pins the fact that the engine's identity is Dilithium-derived. If
    /// `KeyPair::public_key()`/`sign()` ever change algorithm, this fails loudly so the
    /// validator_set construction is revisited (an ed25519-built set would silently
    /// never match vote.voter and break consensus).
    #[test]
    fn engine_address_is_dilithium_derived() {
        let kp = KeyPair::generate();
        // What BftEngine actually uses for vote.voter:
        let engine_addr = consensus_address(&kp.public_key());
        // Must equal deriving from the Dilithium key, and NOT the ed25519 key.
        ...
        // And the helper we'll use for peers agrees, given the RAW Dilithium key_data
        // (as_bytes = untagged key_data; to_vec adds a [alg][len] prefix and must NOT
    ...
    fn two_hosts_finalize_in_lockstep() {
        ...
        let set: Vec<Address> = kps.iter().map(|k| consensus_address(&k.public_key())).collect();
